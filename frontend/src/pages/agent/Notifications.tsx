...
const Notifications = () => {
  ...
  const [activeTab, setActiveTab] = useState<"followups" | "works">(
    "followups",
  );

  const CACHE_KEY = "agentNotificationsCache:v1";
  ...
  const loadFromCache = () => {
    try {
      ...
      const parsed = JSON.parse(cached) as {
        ...
        leadsToFollowUp: Lead[];
        collegesToFollowUp: College[];
      };
      if (Date.now() - parsed.timestamp > CACHE_TTL_MS) return false;
      setWorkStats(parsed.workStats);
      setLeadsToFollowUp(parsed.leadsToFollowUp || []);
      setCollegesToFollowUp(parsed.collegesToFollowUp || []);
      setIsLoading(false);
      return true;
    } catch {
      return false;
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#FAF9F6" }}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex space-x-4 mb-6">
          ...
          >
            Follow-up Reminders (
            {leadsToFollowUp.length + collegesToFollowUp.length})
          </button>
          <button
                      ...
                      {collegesToFollowUp.map((college) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-4">
                            <button
                              onClick={() =>
                                navigate(`/agent/colleges/${college.id}`)
                              }
              ...
              collegesToFollowUp.length === 0 && (
                <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
                  ...
                  <p className="mt-2 text-sm text-gray-500">
                    You're all caught up! No leads or colleges need follow-up
                    today.
                  </p>
                </div>
              )}
            ...
            {workStats &&
              ...
              workStats.recentCompletedWorks.length === 0 && (
                <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
                  ...
                  <p className="mt-2 text-sm text-gray-500">
                    You're all caught up! No pending works at the moment.
                  </p>
                </div>
              )}
          </>
